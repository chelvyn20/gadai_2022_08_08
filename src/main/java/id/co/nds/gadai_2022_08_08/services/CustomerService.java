...
public class CustomerService implements Serializable {
    ...
    public CustomerEntity doInsert(CustomerModel customerModel) throws ClientException {
        ...
        customer.setCustStatus(GlobalConstanst.REC_STATUS_ACTIVE);

        return customerRepo.save(customer);
    }

    public CustomerEntity doUpdatePelanggan(CustomerModel customerModel)
            throws ClientException, NoSuchElementException {
    ...
    public CustomerEntity doDeletePelanggan(CustomerModel customerModel)
            throws ClientException, NoSuchElementException {
        if (!customerRepo.existsById(customerModel.getCustId())) {
            throw new NoSuchElementException("Cannot find customer with id: " + customerModel.getCustId());
        }
        ...
        if (customer.getCustStatus().equalsIgnoreCase(GlobalConstanst.REC_STATUS_NON_ACTIVE)) {
            throw new ClientException("Customer id (" + customerModel.getCustId() + ") has already been deleted");
        }

        customer.setCustStatus(GlobalConstanst.REC_STATUS_NON_ACTIVE);
